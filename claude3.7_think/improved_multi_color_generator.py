#!/usr/bin/env python3
"""
Improved Multi-Color LTX Ball Sequence Generator

This script creates .prg files for complex color sequences with multiple colors and
variable durations. It's based on analysis of working examples, particularly the
2px_red_5_green_10_blue_15.prg file which demonstrates different durations for each color.
"""
...
def generate_multi_color_prg(pixel_count, colors, durations, output_file):
    ...
    # Calculate total header size (16 bytes fixed + variable header + segment descriptors)
    # Each segment descriptor is approximately 16 bytes
    header_size = 16 + 16 + (segment_count * 16)
    ...
    # Offset to first segment's color data (header size + a small constant)
    first_segment_offset = header_size + 4  # Adding a small offset
    ...
    for i, duration in enumerate(durations):
        # First segment has a different format
        if i == 0:
            ...
            # Offset to next segment's color data
            next_offset = color_data_offset + color_block_size
            ...
            if i < segment_count - 1:
                segments.extend((0x0).to_bytes(1, byteorder='little'))
        
        # Last segment has a different format
            ...
            # Offset to this segment's color data
            current_offset = color_data_offset + (i * color_block_size)
            segments.extend(current_offset.to_bytes(2, byteorder='little'))
        ...
        # Middle segments
            ...
            # Offset to next segment's color data
            next_offset = color_data_offset + ((i + 1) * color_block_size)
            segments.extend(next_offset.to_bytes(2, byteorder='little'))
    ...
    # --- 5. Footer ---
    # Adjust footer based on number of colors
    if segment_count == 2:
