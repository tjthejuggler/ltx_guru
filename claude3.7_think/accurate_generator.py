#!/usr/bin/env python3
"""
Accurate LTX Ball Sequence Generator

This script creates .prg files that exactly match the format of working LTX ball programs.
Based on hex analysis of successfully working .prg files.
"""
...
def generate_single_color_prg(pixel_count, rgb_color, output_file):
    """
    Generate a .prg file for a single solid color.
    ...
    var_header.extend((0x64).to_bytes(2, byteorder='little'))  # Duration: 100 (10 seconds)
    ...
    segment.extend((0x64).to_bytes(2, byteorder='little'))         # Duration: 100 (10 seconds)
    segment.extend((0x0).to_bytes(2, byteorder='little'))          # Unknown zeros
...
def generate_two_color_prg(pixel_count, rgb_color1, rgb_color2, output_file):
    """
    Generate a .prg file for a two-color sequence.
    ...
    segment1.extend((0x0a).to_bytes(2, byteorder='little'))        # Duration for this segment
    segment1.extend((0x0).to_bytes(2, byteorder='little'))         # Zeros
    segment1.extend((0x0).to_bytes(2, byteorder='little'))         # Zeros
    segment1.extend((0x64).to_bytes(2, byteorder='little'))        # Overall duration: 100
    ...
    segment2.extend((0x0a).to_bytes(2, byteorder='little'))        # Duration for this segment
    segment2.extend((0x0).to_bytes(2, byteorder='little'))         # Zeros
